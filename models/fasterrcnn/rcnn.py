...
class RCNN(snt.AbstractModule):
    ...
    def loss(self, prediction_dict):
        ...
        with tf.name_scope('RCNNLoss'):
            ...
            '''
            return {
                "rcnn_cls_loss": tf.cond(prediction_dict["with_classification_prediction"],
                            true_fn=lambda :tf.reduce_mean(cross_entropy_per_proposal),
                            false_fn=lambda :tf.constant(0.0, dtype=tf.float32)),
                "rcnn_reg_loss": tf.cond(prediction_dict["with_classification_prediction"],
                            true_fn=lambda :tf.reduce_mean(reg_loss_per_proposal),
                            false_fn=lambda :tf.constant(0.0, dtype=tf.float32)
                )
            }
            '''
