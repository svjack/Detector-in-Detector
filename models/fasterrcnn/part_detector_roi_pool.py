...
class ROIPoolingLayer(snt.AbstractModule):
    ...
    def __init__(self, config, debug=False, name='roi_pooling'):
        ...
        self._pooled_width = 5
        self._pooled_height = 5
    ...
    def _roi_crop(self, roi_proposals, conv_feature_map, im_shape):
        ...
        valid_indicator = tf.reduce_sum(tf.reshape(tf.nn.relu(tf.sign(conv_feature_map)), [-1]), axis=0)
        ...
        crops = tf.cond(
            tf.greater(valid_indicator, 0.0),
            true_fn=lambda :tf.image.crop_and_resize(
                conv_feature_map, bboxes, batch_ids,
                [self._pooled_width * 2, self._pooled_height * 2], name="crops"
            ),
            false_fn=lambda : tf.zeros(shape=[1,self._pooled_width * 2, self._pooled_height * 2, 256],
                                       dtype=tf.float32)
        )
